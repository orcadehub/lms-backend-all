...
const createMazeGame = async () => {
  try {
    ...
    await GamifiedQuestion.deleteMany({ title: 'Maze Challenge - Hard (7x7)' });

    const levels = [];
    for (let i = 1; i <= 20; i++) {
      levels.push({
        levelNumber: i,
        question: `Navigate through the 7x7 maze from red to green (Level ${i})`,
        questionType: 'Interactive',
        options: [],
        correctAnswer: JSON.stringify({ type: 'maze', rows: 7, cols: 7 }),
        pointsForLevel: 2,
        timeLimit: 90,
    ...
    const mazeGame = new GamifiedQuestion({
      ...
      title: 'Maze Challenge - Hard (7x7)',
      description: 'Navigate through 7x7 mazes',
      difficulty: 'Hard',
      points: 40,
      gameType: 'MCQ',
      isMultiLevel: true,
      ...
      maxScore: 40,
      passingScore: 24,
      speedBonus: { enabled: true, maxBonus: 20, timeThreshold: 400 },
      streakBonus: { enabled: true, bonusPerStreak: 5 },
      isActive: true
    });

    await mazeGame.save();
    console.log('7x7 Maze game created successfully with 20 levels');
