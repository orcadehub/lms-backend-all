...
const mongoDBStudentsQuestions = [
  {
  ...
  {
    title: "Find Female Students from CSE Branch with Projection",
  ...
  },
  {
    ...
    problemStatement: "Write a MongoDB aggregation query to find the average maths score for each gender. Round the average to 2 decimal places.",
    collectionName: "students",
    starterCode: "",
    testCases: [
      {
        description: "Should return average maths score grouped by gender with 2 decimals",
        expectedQuery: "db.students.aggregate([{ $group: { _id: '$gender', avgMaths: { $avg: '$maths' } } }, { $project: { _id: 1, avgMaths: { $round: ['$avgMaths', 2] } } }])",
        isPublic: true
      }
    ],
    difficulty: "hard",
  ...
  {
    title: "Find Maximum and Minimum Physics Score by Branch",
    problemStatement: "Write a MongoDB aggregation query to find the maximum and minimum physics score for each branch.",
    collectionName: "students",
    starterCode: "",
    testCases: [
      {
        description: "Should return max and min physics score by branch",
        expectedQuery: "db.students.aggregate([{ $group: { _id: '$branch', maxPhysics: { $max: '$physics' }, minPhysics: { $min: '$physics' } } }])",
        isPublic: true
      }
    ],
    difficulty: "hard",
  ...
  {
    title: "Find Average Chemistry Score by City for Students with Attendance Greater Than 95",
    problemStatement: "Write a MongoDB aggregation query to match only students with attendance > 95, then calculate average chemistry score for each city. Round the average to 2 decimal places.",
    collectionName: "students",
    starterCode: "",
    testCases: [
      {
        description: "Should return average chemistry by city for attendance > 95 with 2 decimals",
        expectedQuery: "db.students.aggregate([{ $match: { attendance: { $gt: 95 } } }, { $group: { _id: '$city', avgChemistry: { $avg: '$chemistry' } } }, { $project: { _id: 1, avgChemistry: { $round: ['$avgChemistry', 2] } } }])",
        isPublic: true
      }
    ],
    difficulty: "hard",
...
async function seedMongoDBStudentsQuestions() {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('Connected to MongoDB');

    const insertedQuestions = await MongoDBPlaygroundQuestion.insertMany(mongoDBStudentsQuestions);
    console.log(`Inserted ${insertedQuestions.length} MongoDB students questions`);

    await mongoose.connection.close();
    console.log('Database connection closed');
  } catch (error) {
    console.error('Error seeding MongoDB students questions:', error);
    process.exit(1);
  }
}

seedMongoDBStudentsQuestions();
