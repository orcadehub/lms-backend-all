...
const createMazeGame = async () => {
  try {
    ...
    await GamifiedQuestion.deleteMany({ title: 'Maze Challenge - Easy (5x5)' });

    const levels = [];
    for (let i = 1; i <= 20; i++) {
      levels.push({
        levelNumber: i,
        question: `Navigate through the 5x5 maze from red to green (Level ${i})`,
    ...
    const mazeGame = new GamifiedQuestion({
      ...
      description: 'Navigate through 5x5 mazes',
      difficulty: 'Easy',
      points: 40,
      gameType: 'MCQ',
      isMultiLevel: true,
    ...
    await mazeGame.save();
    console.log('5x5 Maze game created successfully with 20 levels');
